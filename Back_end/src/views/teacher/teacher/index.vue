<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="登录id,关联到登录账号" prop="loginId">
        <el-input
          v-model="queryParams.loginId"
          placeholder="请输入登录id,关联到登录账号"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="teacherList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="登录id,关联到登录账号" align="center" prop="loginId" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="teacherRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="登录id,关联到登录账号" prop="loginId">
          <el-input v-model="form.loginId" placeholder="请输入登录id,关联到登录账号" />
        </el-form-item>
      </el-form>
      <template #footer>
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除teacher编号为"' + _ids + '"的数据项？').then(function() {
